# 2月21日
## Doing
今天是回学校后第一天继续写毕设，成果如下：
对于活跃事务的维护用一个红黑树的Set实现，这样可以以可以接受的代价插入事务和移除事务并获得活跃事务上下限供GC使用；
AOF编码解码部分推翻了之前的encode思路，直接用fstream的write/read方法读写二进制文件，将AOF结构体写入，每100条AOF操作追加写一次；
AOF操作的缓冲区依然决定用rust的mpsc实现，所以研究了如何在cpp中调用rust代码，最后发现在wsl上安装rust后会非常方便，windows有太多环境问题（路径、链接、编译器ABI等）。
## Todo
实现完TXManager的所有部分；
掌握用cmake在linux上混合编译rust和cpp代码的方法。
